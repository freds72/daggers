...
function make_voxel_editor()   
    ...
    return is_window({
        ...
        undo=function(self,msg)      
            ...
            local prev=deli(undo_stack)
            apply(prev.idx,prev.col)
            if(#undo_stack==0) _dirty=nil
        end,
...
function unpack_archive()
    -- check magic number
    local mem=0x0
    if($mem!=_magic_number) printh("archive: invalid magic number") _reload() return unpack_archive()
    mem+=4
...
function confirm_reset()
    ...
    menuitem(5,"reset all - ok?",function()
        _reload()
        -- kill old archive
        cstore(0x0,0x0,0x4300,"freds72_daggers_assets.p8")
        unpack_archive()
        reload()
        reload_entity()
        menuitem(5,"reset all",confirm_reset)
...
load;#freds72_daggers_title]]
    ...
    left_panel:add(make_button("MODELS\152",binding(function()
        ...
        left_panel:add(make_button("BACK\022",binding(function() dialog:close() end)))
        local lists={
...
load;#freds72_daggers_title_mini]]            
    ...
    right_panel:add(make_button("\138TO TITLE",binding(function()
        if _dirty then
            local dialog=_main:dialog(true)
            local s="UNSAVED WORK! EXIT?"        
